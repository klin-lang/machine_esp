...
`UartLp` / `I2c` / `Spi` / `Adc` / S3 `Rmt` use explicit MMIO** (ESP32-C3 / ESP32-S3 /
ESP32-P4 GPIO + LP GPIO + IO_MUX + LEDC + UART/LP UART/I2C/SPI2/ADC1 + RMT). Boot and flash use
...
| **ESP32-P4** | `pin_out_p4` / `pin_in_p4` / `pin_out_lp_p4` | `pwm_out_p4` | `rc_out_p4` | `uart_out_p4` / `uart_out_lp_p4` | `i2c_out_p4` | `spi_out_p4` | `adc_out_p4` / `adc2_out_p4` | `rmt_tx_p4` | — | `*_p4` | ESP-IDF v5.x |
...
`version()` → `15` (`@v0.15.0`). P4 LP UART: `uart_out_lp_p4` (base `0x50121000`, type `UartLp`). P4 SAR regi2c: `adc_cal_init_p4` (DREF; LP_I2C_ANA_MST `0x50124000`, slave `0x69`). LP GPIO 0..15: `pin_out_lp_p4`. P4 ADC oneshot is **LP_ADC** — call **`read_u12_p4`**. ADC2: `adc2_out_p4`.
...
| Peri clocks | SYSTEM `0x600C…` | SYSTEM `0x600C…` | **HP_SYS_CLKRST** `0x500E6000` (ADC / LP UART / LP IOMUX: **LPPERI** `0x50120000`) |
| LEDC timer | `freq` (C3/S3 layout) | `freq` | **`freq_p4`** (RST bit 24, CONF `@ +0x170`) |
| ADC oneshot | APB_SARADC + `read_u12` | APB_SARADC + `read_u12` | **LP_ADC** + **`read_u12_p4`** (ADC1 CH0→GPIO16; ADC2 `adc2_out_p4` CH0→GPIO49); regi2c `adc_cal_init_p4` |
...
machine_esp/
  ...
  uart.kl / uart_s3.kl / uart_p4.kl / uart_lp_p4.kl / i2c.kl / i2c_s3.kl / i2c_p4.kl
  spi.kl / spi_s3.kl / spi_p4.kl / adc.kl / adc_s3.kl / adc_p4.kl / adc_cal_p4.kl
...
let lp_u = machine.uart_out_lp_p4(14, 15, 20000000, 115200)  // LP UART; UartLp
...
machine.adc_cal_init_p4()                    // regi2c DREF=4; not inside adc_out_p4
...
P4 LP UART: **`uart_out_lp_p4`** — one instance, LP GPIO 0..15, sclk **XTAL_D2** (pass `20000000` typically). Returns **`UartLp`** (FIFO bits ≠ HP `Uart`). Do not also call `uart_out_p4` / `pin_out_lp_p4` on the same pads.
...
P4 SAR regi2c: **`adc_cal_init_p4`** (DREF=4), **`adc_cal_init_code_p4`**, **`adc_cal_encal_gnd_p4`** — LP_I2C_ANA_MST, not C3/S3 I2C_MST. No eFuse curve-fit.
...
```sh
. $IDF_PATH/export.sh
cd examples/blink_p4   # or uart_lp_p4 / adc_cal_p4 / blink_lp_p4 / blink_s3 / … / blink_c3 / …
